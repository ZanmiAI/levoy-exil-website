...
- [ ] Review the 8 scheduled articles (docx copies shared in chat 2026-09-21) — flag any changes before each publish date
...
- [x] Article series: 8 SEO/AEO articles written, scheduled twice-monthly (2026-09-21, per Daphnee's approval of the 8 topics): Saint Soleil movement (Oct 1), spotting an original (Oct 15), meaning of the suns (Nov 1), 50 years of exhibitions (Nov 15), investing in Haitian art (Dec 1), caring for your painting (Dec 15), inside the studio (Jan 1), 5 Haitian artists to know (Jan 15). All 1,200-1,550 words, fact-checked (Saint Soleil history, Cinq Soleils, Malraux/L'Intemporel, Artcurial 2019 results verified via web search), buyer-intent phrases woven naturally, AEO question headings, gallery/WhatsApp/email CTAs, real artwork links and existing images only. Integrated into site.json; build.py now date-gates articles (published_articles(): only date <= today builds, listed newest-first; sitemap/index/homepage/article pages all gated; BUILD_DATE dynamic). Rebuilt (validation passes), synced, pushed to GitHub main (b187fa2) + gh-pages (676f18f), live-verified: news index shows only the 2 published articles, no future article leaks. Cron 'levoy-article-publisher' (monthly, 1st & 15th ~9:51 AM ET) rebuilds, pushes, live-verifies, notifies Daphnee, and winds down when the queue is empty. Review docx copies: ~/workspace/your_files/levoy-articles/. Tracked item goal_033be1a2c7ea.
- [x] Gallery photos: no-crop, larger, click-to-enlarge (2026-09-21, per Daphnee's feedback on the "More photos" galleries): the gallery grid no longer crops photos into 4:3 frames — every photo now shows in full with its natural shape, so nothing (heads included) gets cut off; tiles made a bit bigger (280px minimum); clicking any photo opens a full-screen lightbox showing the complete uncropped photo (closes on click, X, or Escape). Rebuilt (validation passes), synced to preview, pushed to GitHub main (608bfa8) + gh-pages (0a5cb25).
- [x] Uniform gallery tiles on all 42 artwork pages (2026-09-21, per Daphnee's instruction: consistent photo size without distortion): the "More photos" gallery switched from a masonry layout (mixed photo sizes) to a uniform grid — every photo now displays in an identical 4:3 frame with edge-to-edge fill (object-fit: cover), so no photo is stretched or squished and nothing is skewed; the photos themselves are untouched (originals intact). The main gallery artwork cards already used this uniform-frame approach. Rebuilt (validation passes), synced to preview, pushed to GitHub main (3840dad) + gh-pages (a36c2bb).
- [x] Full old-site photo galleries on all 42 artwork pages (2026-09-21, per Daphnee's instruction): downloaded all 119 product-gallery image references (104 unique files) from the 42 live WooCommerce product pages and added them to every artwork page as a "More photos" masonry gallery — the original canvas photograph first, then every "how it looks in your home" framed room mockup, in the old site's order. 8 images that pixel-duplicated the existing lifestyle shot were skipped on their page to avoid showing the same photo twice. build.py generates 1200px web versions; descriptive alt text on every image. Rebuilt (all validation checks pass), synced to preview, pushed to GitHub main (0afd309) + gh-pages (e72b2fb), Pages build "built", live-verified: Sayluma page shows the "More photos" gallery with all 3 images and gallery image URLs serve directly (1200px web versions). VISUAL QA COMPLETE 2026-09-21: all 104 unique files inspected via contact sheets — 42 primary artwork photographs (first in each gallery) + 62 framed room mockups, no artist photos/detail shots/corrupt/unsuitable images; every gallery[0] pixel-verified against its artwork's primary photo, so both inferred captions are accurate and no rebuild was needed. Enriched manifest: gallery_manifest_enriched.json (per-file classification, alt, caption, source URL, SHA-256, dimensions). NOTE for cutover: the production build in /tmp/levoy-prod must be REGENERATED at cutover time so the galleries go live on levoyexil.com too.
- [x] CUTOVER IN PROGRESS (2026-09-21 ~8:30 PM ET, per Daphnee's explicit approval "let's connect the website to the domain"): production build regenerated with `"baseurl": ""` (the old /tmp copy had been wiped; rebuilt fresh into `public/`, all validation checks pass). 42 old WooCommerce product-URL redirects added (`/product/<slug>/` -> `/artwork/<slug>/`, meta-refresh + canonical + JS) plus a 404.html fallback catching `/shop/product/<slug>` patterns and sending everything else to the gallery. Pushed to gh-pages (11fe522) with `CNAME` = levoyexil.com; custom domain set via Pages API. The old preview URL now redirects to the domain. NEXT: the 5 DNS records in GoDaddy (Daphnee's part — exact records in PRODUCTION-CUTOVER-CHECKLIST.md), then verify https://levoyexil.com + https://www.levoyexil.com, then update the article-publisher cron to verify the live domain.
...
- [x] Prepare the exact DNS records for the cutover (only after her approval) — records ready in PRODUCTION-CUTOVER-CHECKLIST.md; approval given 2026-09-21
